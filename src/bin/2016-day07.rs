...
fn has_abba(bytes_list: &[&[u8]]) -> bool {
    bytes_list.iter().any(|bytes| bytes.windows(4).any(|x| x[0] != x[1] && x[0] == x[3] && x[1] == x[2]))
}
...
fn main() -> Result<()> {
    ...
    let ips = input
        ...
        .map(|line| {
            ...
            let hypernets = ranges.iter().map(|range| &bytes[range.start + 1..range.end - 1]).collect_vec();
    ...
    let result1 = ips.iter().filter(|(hypernets, supernets)| !has_abba(hypernets) && has_abba(supernets)).count();

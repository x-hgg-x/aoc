...
fn mix(numbers: &[i64], n: usize) -> Result<i64> {
    ...
    for _ in 0..n {
        for (index, &number) in numbers.iter().enumerate() {
            ...
            indices.insert((index_index as i64 + number).rem_euclid(indices.len() as i64) as usize, index);
        }
    }
    ...
    Ok([1000, 2000, 3000].iter().map(|offset| numbers[indices[(zero_index_index + offset) % indices.len()]]).sum())
}
...
fn main() -> Result<()> {
    ...
    let mut numbers: Vec<i64> = input.split_ascii_whitespace().map(|x| x.parse()).try_collect()?;

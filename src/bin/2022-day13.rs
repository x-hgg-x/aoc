...
fn cmp_slice(left: &[Item], right: &[Item]) -> Ordering {
    iter::zip(left, right).map(|(l, r)| l.cmp(r)).find(|&ord| ord != Ordering::Equal).unwrap_or_else(|| left.len().cmp(&right.len()))
}
...
fn main() -> Result<()> {
    ...
    let mut packets: Vec<_> = input.lines().filter(|line| !line.is_empty()).map(|line| Item::parse(&mut line.as_bytes())).try_collect()?;

    let result1 = packets.chunks_exact(2).enumerate().filter(|(_, x)| x[0] < x[1]).map(|(index, _)| index + 1).sum::<usize>();
    ...
    let result2 = dividers.iter().map(|d| packets.iter().filter(|&x| x <= d).count()).product::<usize>();

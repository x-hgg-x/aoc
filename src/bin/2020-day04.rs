...
fn main() -> Result<()> {
    ...
    let iter = input.split("\n\n").filter_map(|passport| {
        let mut fields: SmallVec<[_; 7]> =
            passport.split_ascii_whitespace().filter_map(|x| x.split(':').next_tuple()).filter(|&(name, _)| name != "cid").collect();
    ...
    for fields in iter {
        let [(_, byr), (_, ecl), (_, eyr), (_, hcl), (_, hgt), (_, iyr), (_, pid)] = <[_; 7]>::try_from(fields.as_slice())?;

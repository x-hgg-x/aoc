...
fn generator(subject_number: u64) -> impl Iterator<Item = u64> {
    std::iter::successors(Some(1), move |x| Some((x * subject_number) % 20201227))
}
...
fn main() -> Result<()> {
    ...
    let (card_public_key, door_public_key) =
        input.split_ascii_whitespace().map(|x| Ok(x.parse::<u64>()?)).try_process(|mut iter| iter.next_tuple())?.value()?;

...
fn main() -> Result<()> {
    ...
    let list_with_count = list
        .into_iter()
        .dedup_with_count()
        .map(|(count, elem)| (count as i64, elem))
        .collect_vec();
    ...
    let (min_cost_1, min_cost_2) = (min..=max)
        ...
        .fold(
            (i64::MAX, i64::MAX),
            |(min_cost_1, min_cost_2), (cost1, cost2)| {
                (min_cost_1.min(cost1), min_cost_2.min(cost2))
            },
        );

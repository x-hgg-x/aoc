...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let edges = HashMap::<_, _>::from_iter(re.captures_iter(&input).flat_map(|cap| {
        ...
        [((cap[1].to_owned(), cap[2].to_owned()), distance), ((cap[2].to_owned(), cap[1].to_owned()), distance)]
    }));

    let distances =
        nodes.iter().permutations(nodes.len()).map(|x| -> u32 { x.windows(2).map(|x| edges[&((*x[0]).into(), (*x[1]).into())]).sum() }).collect_vec();

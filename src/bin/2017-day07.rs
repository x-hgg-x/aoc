...
fn compute_total_weights<'a>(
    ...
    let mut queue: VecDeque<_> = nodes
        .iter()
        .filter(|&(_, node)| node.children_names.is_empty())
        .map(|(&name, _)| name)
        .collect();
    ...
    while let Some(name) = queue.pop_front() {
        ...
        let total_weight = (node.children_names.iter()).fold(node.weight, |acc, &child_name| {
            acc + total_weights[child_name]
        });

        total_weights.insert(name, total_weight);
...
fn compute_unbalanced_node_corrected_weight(
    nodes: &HashMap<&str, Node>,
    bottom_node: &Node,
    total_weights: &HashMap<&str, u64>,
) -> Result<u64> {
    let mut unbalanced_node_name = bottom_node.name;
    let mut balanced_weight = bottom_node.weight;

    loop {
        let mut children_total_weights: SmallVec<[_; 8]> = nodes[unbalanced_node_name]
            .children_names
            .iter()
            .map(|&child_name| (child_name, total_weights[child_name]))
            .collect();

        children_total_weights.sort_unstable_by_key(|&(_, weight)| weight);

        let mut iter = children_total_weights
            .iter()
            .dedup_by_with_count(|&(_, weight_1), &(_, weight_2)| weight_1 == weight_2);

        match (iter.next(), iter.next()) {
            (Some((1, &(child_name, _))), Some((_, &(_, weight))))
            | (Some((_, &(_, weight))), Some((1, &(child_name, _)))) => {
...
fn main() -> Result<()> {
    ...
    for cap in regex_line.captures_iter(&input) {
        ...
        let children_names = regex_children
            .find_iter(cap.get(3).value()?.as_str())
            .map(|x| x.as_str())
            .collect();
    ...
    let result2 =
        compute_unbalanced_node_corrected_weight(&nodes, &nodes[bottom_node_name], &total_weights)?;

...
fn check_rules_mapping(mut rule_indices: SmallVec<[Option<u64>; 20]>, possibilities: &[u64]) -> Result<()> {
    rule_indices.sort_unstable();

    ensure!(possibilities.iter().all(|&x| x == 0), "unable to map rules to fields");
    ensure!(rule_indices.iter().enumerate().all(|(index, &x)| x == Some(index as u64)), "unable to map rules to fields");
...
fn main() -> Result<()> {
    ...
    let (rules_input, self_ticket_input, nearby_tickets_input) = input.split("\n\n").next_tuple().value()?;
    ...
    let self_ticket: SmallVec<[u64; 20]> = self_ticket_input.lines().last().value()?.split(',').map(|x| x.parse()).try_collect()?;

    let mut nearby_tickets: Vec<SmallVec<[u64; 20]>> =
        nearby_tickets_input.lines().skip(1).map(|line| line.split(',').map(|x| x.parse()).try_collect()).try_collect()?;

    let min_rule = rules.iter().flat_map(|(_, ranges)| ranges).map(|x| *x.start()).min().value()?;
    let max_rule = rules.iter().flat_map(|(_, ranges)| ranges).map(|x| *x.end()).max().value()?;

    let mut invalid_values = (min_rule..=max_rule).map(Some).collect_vec();

    for range in rules.iter().flat_map(|(_, ranges)| ranges) {
        invalid_values[(range.start() - min_rule) as usize..=(range.end() - min_rule) as usize].fill(None);
    }
    ...
    nearby_tickets.retain(|ticket| {
        let sum = ticket.iter().filter(|&&value| value < min_rule || invalid_values.contains(&value) || value > max_rule).sum1::<u64>();
        if let Some(sum) = sum {
    ...
    let mut possibilities = SmallVec::<[_; 20]>::from_elem(!0u64 >> (u64::BITS - rules.len() as u32), rules.len());
    ...
    while let Some((index, &possibility)) = possibilities.iter().find_position(|possibility| possibility.count_ones() == 1) {

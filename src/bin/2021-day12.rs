...
fn count_paths<'a>(
    ...
    queue.push(State {
        path: SmallVec::from_slice(&[idx_start]),
        neighbors: &graph[&idx_start][..],
        twice,
    });
...
fn main() -> Result<()> {
    ...
    let edges: Vec<(_, _)> = input
        .lines()
        .map(|line| line.split('-').next_tuple().value())
        .try_collect()?;
    ...
    let mapping: HashMap<_, _> = nodes
        .iter()
        .enumerate()
        .map(|(index, &node)| (node, index as u8))
        .collect();
